...
wget.callbacks.download_child_p = function(urlpos, parent, depth, start_url_parsed, iri, verdict, reason)
  ...
  if string.match(url, "^https?://[^/]+%.ua/") then
    queue_url(queued_outlinks_ua, url)
  else
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, items_data in pairs({
    ["ukr-net-ki9yued7bvzzyjj"]=queued_urls,
    ["ukr-net-urls-p1jpiftsgrffogk"]=queued_outlinks,
    ["ua-ieqqxz6l3beowx6"]=queued_outlinks_ua
  }) do
    ...
    for url, _ in pairs(items_data) do
      if (name ~= "ukr-net" and not queued_urls[url]) or name == "ukr-net" then
